...
1. **Given** I am on the endpoints page, **When** I click "New Endpoint" and fill in name, target URL, port, and path, **Then** a new endpoint is created with an auto-generated slug
2. **Given** I have created an endpoint, **When** I view the endpoints list, **Then** I see the endpoint with its name, full webhook URL, and status
3. **Given** an endpoint is displayed, **When** I click the copy button next to the webhook URL, **Then** the full URL (e.g., `https://hookspy.vercel.app/api/hook/abc123`) is copied to my clipboard
4. **Given** I am creating an endpoint, **When** I leave the target URL empty, **Then** it defaults to `http://localhost`
5. **Given** I am creating an endpoint, **When** I leave the port empty, **Then** it defaults to `3000`
...
- What happens when the user tries to edit an endpoint that was just deleted? API returns 404, UI shows an error toast
- What happens with a very long custom header value? No validation limit; Postgres text handles it

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: System MUST provide an API endpoint `GET /api/endpoints` that returns all endpoints for the authenticated user
- **FR-002**: System MUST provide an API endpoint `POST /api/endpoints` that creates a new endpoint with auto-generated slug (8-char hex from UUID)
- **FR-003**: System MUST provide an API endpoint `PUT /api/endpoints/:id` that updates an existing endpoint (only if owned by authenticated user)
- **FR-004**: System MUST provide an API endpoint `DELETE /api/endpoints/:id` that deletes an endpoint (only if owned by authenticated user)
- **FR-005**: System MUST provide an endpoints list view at `/endpoints` displaying all user's endpoints as cards
- **FR-006**: System MUST provide an endpoint detail/edit view at `/endpoints/:id` with a form for all configurable fields
...
- **FR-008**: The endpoint form MUST include fields: name (text), target URL (text, default `http://localhost`), target port (number, default 3000), target path (text, default `/`), timeout (number slider, range 1-55, default 30)
...
- **FR-011**: System MUST implement a Pinia store (`stores/endpoints.js`) for endpoint state management
- **FR-012**: Delete MUST show a PrimeVue ConfirmDialog before proceeding
